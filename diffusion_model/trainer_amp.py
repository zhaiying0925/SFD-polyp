...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, condition_tensors=None, style_img=None, noise=None):
        ...
        # x_in = torch.cat([x_noisy, condition_tensors], 1)
        # print("x_in.shape (拼接后):", x_in.shape)
        x_recon = self.denoise_fn(torch.cat([x_noisy, condition_tensors], 1), t, style_img=style_img)

        # if style_feat is not None:
        #     x_recon = self.denoise_fn(torch.cat([x_noisy, condition_tensors], 1), t, style_feat=style_feat)
        # else:
    ...
    def forward(self, x, condition_tensors=None, style_img=None, *args, **kwargs):
        b, c, h, w, device, img_size, = *x.shape, x.device, self.image_size
        assert h == img_size and w == img_size, f'height and width of image must be {img_size}'
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        return self.p_losses(x, t, condition_tensors=condition_tensors, style_img=style_img, *args, **kwargs)
...
class Trainer(object):
    def __init__(
        ...
        super().__init__()
        self.model = diffusion_model.cuda()
        self.ema = EMA(ema_decay)
        self.ema_model = copy.deepcopy(self.model).cuda()
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                ...
                batch_size = target_tensors.shape[0]

                # 随机打乱 batch 索引
                rand_indices = torch.randperm(batch_size).cuda()
                style_tensors = target_tensors[rand_indices]

                # print("44444444444444444")
                # print(input_tensors.shape)

                # print("111111111111111111")
                # print(style_tensors.shape)

                # style_tensors = target_tensors.clone()

                with torch.cuda.amp.autocast(enabled=self.amp):
                    # loss = self.model(target_tensors, condition_tensors=input_tensors)
                    loss = self.model(target_tensors, condition_tensors=input_tensors, style_img=style_tensors)
